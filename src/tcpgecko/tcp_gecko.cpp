...
void considerInitializingFileSystem()
{
	if (!client)
	{
		...
		// Register the client
		FSStatus status = FSAddClient(client, FS_ERROR_FLAG_ALL);
		ASSERT_FUNCTION_SUCCEEDED(status, "FSAddClient");

		// Allocate the command block
		commandBlock = new FSCmdBlock;
...
static int processCommands(struct pygecko_bss_t* bss, int clientfd)
{
	...
	while (true)
	{
		...
		case COMMAND_MEMORY_DISASSEMBLE: {
			...
			while (currentAddress < endingAddress)
			{
					...
					&& (currentAddress < endingAddress))
				{
					...
					bool status = DisassemblePPCOpcode(
						(u32*)currentAddress, opCodeBuffer, (u32)bufferSize,
						OSGetSymbolName,
		...
		case COMMAND_READ_FILE: {
			...
			receiveString(bss, clientfd, (unsigned char*)file_path, FS_MAX_PATH);
			...
			if (status == FS_STATUS_OK)
			{
				...
				{
					int bytesRead = FSReadFile(
						client, commandBlock, (u8*)fileBuffer, 1, file_buffer_size,
		...
		case COMMAND_READ_DIRECTORY: {
			char directory_path[FS_MAX_PATH] = { 0 };
			receiveString(bss, clientfd, (unsigned char*)directory_path, FS_MAX_PATH);
			...
			if (ret == FS_STATUS_OK)
			{
				...
				while (FSReadDir(client, commandBlock, handle, &entry, FS_ERROR_FLAG_ALL) == FS_STATUS_OK)
				{
					...
					while (true)
					{
						...
						ASSERT_FUNCTION_SUCCEEDED(ret, "recvwait (File bytes length)")
							...
							if (dataLength > 0)
							{
								...
								ret = FSWriteFile(
									client, commandBlock, (u8*)fileBuffer, 1,
		...
		case COMMAND_READ_THREADS: {
			std::vector<OSThread*> threads = getAllThreads();
			...
			{
				// log_printf("Thread data: %08x\n", data);
				((int*)buffer)[0] = (int)thread;
				memcpy(buffer + sizeof(int), (void*)thread, sizeof(OSThread));
				log_print("Sending node...\n");
				ret = sendwait(bss, clientfd, buffer, sizeof(int) + sizeof(OSThread));
				ASSERT_FUNCTION_SUCCEEDED(ret, "sendwait (thread address and data)");
			}
		...
		case COMMAND_ACCOUNT_IDENTIFIER: {
			...
			// Get the identifier
			ret = nn::act::Initialize();
			// ASSERT_INTEGER(ret, 1, "Initializing account library");
			unsigned char slotNumber = nn::act::GetSlotNo();
			unsigned int persistentIdentifier = nn::act::GetPersistentIdEx(slotNumber);
			ret = nn::act::Finalize();
...
int sockfd = -1, clientfd = -1, ret = 0;
socklen_t len;
struct sockaddr_in socketAddress;
struct pygecko_bss_t* bss;

static int runTCPGeckoServer(int argc, const char** argv)
{
	...
	while (true)
	{
		...
		CHECK_ERROR(ret < 0)
			...
			while (true)
			{
				...
				clientfd = accept(sockfd, (struct sockaddr*)&socketAddress, &len);
...
static int startTCPGeckoThread(int argc, const char** argv)
{
	...
	if (OSCreateThread(bss->thread, runTCPGeckoServer, 1, (char*)bss,
		(void*)(bss->stack + sizeof(bss->stack)),
		sizeof(bss->stack), 0,
		0xc) == 1)
	{

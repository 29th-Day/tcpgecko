...
static int sd_fat_open_r(struct _reent* r, void* fileStruct, const char* path, int flags, int mode)
{
	...
	FSStatus result = FSOpenFile(dev->pClient, dev->pCmd, real_path, mode_str, &fd, FS_ERROR_FLAG_ALL);
...
static int sd_fat_stat_r(struct _reent* r, const char* path, struct stat* st)
{
	...
	FSStatus result = FSGetStat(dev->pClient, dev->pCmd, real_path, &stats, -1);
...
static const devoptab_t devops_sd_fat = {
		...
		NULL, // sd_fat_fpathconf_r
		NULL, // sd_fat_pathconf_r
		NULL, // sd_fat_symlink_r
		NULL, // sd_fat_readlink_r
};
...
int mount_sd_fat(const char* path)
{
	...
	FSInit();
	FSInitCmdBlock(pCmd);
	FSAddClient(pClient, FS_ERROR_FLAG_ALL);
...
int unmount_sd_fat(const char* path)
{
	...
	if (result == 0)
	{
		...
		FSDelClient(pClient, FS_ERROR_FLAG_ALL);
		free(pClient);
		free(pCmd);
		free(mountPath);
		//FSShutdown();

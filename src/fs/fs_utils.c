...
int MountFS(FSClient* pClient, FSCmdBlock* pCmd, char** mount_path)
{
	...
	FSMountSource* mountSrc = calloc(0, sizeof(FSMountSource));
	if (!mountSrc)
		return -3;

	char mountPath[FS_MAX_PATH];

	// memset(mountSrc, 0, FS_MOUNT_SOURCE_SIZE);
	// memset(mountPath, 0, FS_MAX_PATH);

	// Mount sdcard
	result = FSGetMountSource(pClient, pCmd, FS_MOUNT_SOURCE_SD, mountSrc, -1);
	if (result == 0)
	{
		result = FSMount(pClient, pCmd, mountSrc, mountPath, FS_MAX_PATH, -1);

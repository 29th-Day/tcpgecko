...
static unsigned char exceptionCallback(OSContext* context, unsigned char exception_type)
{
	...
	pos += sprintf(
		stackTraceBuffer + pos, exception_print_formats[10],
		context->dar,
		context->dsisr); // this freezes
	//}

	void* pc = (void*)context->srr0;
	void* lr = (void*)context->lr;
	void* r1 = (void*)context->gpr[1];
	u32 currentStackTraceDepth = 0;
...
int dsi_exception_cb(OSContext* context)
{
	return exceptionCallback(context, 0);
}

int isi_exception_cb(OSContext* context)
{
	return exceptionCallback(context, 1);
}

int program_exception_cb(OSContext* context)
{
...
void setup_os_exceptions()
{
	OSSetExceptionCallback(
		OS_EXCEPTION_TYPE_DSI, &dsi_exception_cb);
	OSSetExceptionCallback(
		OS_EXCEPTION_TYPE_ISI, &isi_exception_cb);
	OSSetExceptionCallback(
		OS_EXCEPTION_TYPE_PROGRAM, &program_exception_cb);
}

	...
	volatile unsigned int* dynamic_method_calls)
{
	...
	for (int functionIndex = 0; functionIndex < method_hooks_count; functionIndex++)
	{
		...
		{
			if (isDynamicFunction(OSEffectiveToPhysical(functionHook[functionIndex].realAddress)))
			{
		...
		physical = OSEffectiveToPhysical(realAddress);
		if (!physical)
		{
		...
		*space = 0x4E800420; // bctr
		...
		DCFlushRange(&replace_instr, sizeof(replace_instr));

		SC0x25_KernelCopyData(physical, OSEffectiveToPhysical(replace_instr), 4);
...
void restoreIndividualInstructions(FunctionHook* functionHooks, int hook_information_size)
{
	...
	for (int functionIndex = 0; functionIndex < method_hooks_count; functionIndex++)
	{
		...
		u32 physical = OSEffectiveToPhysical(realAddress);
		if (!physical)
		{
		...
		{
			physical = OSEffectiveToPhysical(functionHooks[functionIndex].realAddress); //When its an static function, we need to use the old location
			if (DEBUG_LOG_DYN)
...
OSDynLoad_Module getRPLHandle(int library, const char* functionName)
{
	OSDynLoad_Module rplHandle = 0;
	OSDynLoad_Module gx2_handle = 0;
	...
	case LIB_CORE_INIT:
		...
		rplHandle = (OSDynLoad_Module)coreinit_handle;
		break;
...
unsigned int getFunctionAddress(unsigned int library, const char* functionName)
{
	...
	{
		memcpy(&realAddress, (void*)&OSDynLoad_Acquire, 4);
		return realAddress;
	}
	...
	OSDynLoad_Module rpl_handle = getRPLHandle(library, functionName);
	...
	OSDynLoad_FindExport(rpl_handle, OS_DYNLOAD_EXPORT_FUNC, functionName, (void**)&realAddress);
